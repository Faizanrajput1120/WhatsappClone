...
const Sidebar = () => {
  ...
  const setChat = async (user) => {
    ...
    try {
      ...
      // ENFORCE CHAT RESTRICTIONS:
      // - Admin can chat with anyone.
      // - Normal user can ONLY chat with Admins.
      if (!isGlobalAdmin) {
        let otherUserData = user.userData; // if passed from chat list
        if (!otherUserData) {
          const snap = await getDoc(doc(db, "users", otherUid));
          otherUserData = snap.data();
        }
        const otherIsAdmin = otherUserData?.role === 'admin' || otherUserData?.isAdmin === true;
        if (!otherIsAdmin) {
          toast.error("Normal users can only chat with admins.");
          return;
        }
      }

      const msgId = getMessagesId(userData.uid, otherUid);
      ...
      if (!querySnap.empty) {
        const results = querySnap.docs
          ...
          .filter((user) => {
            ...
            // Normal user can only see admins
