...
export const useWebRTCCall = (userId, isAdmin = false) => {
  ...
  const startCall = useCallback(async (receiverId, callType = 'video') => {
    if (!userId || callState !== CALL_STATES.IDLE ) return;
    ...
    try {
      ...
      if (!isAdmin) {
        ...
        const receiverIsAdmin = receiverData?.role === 'admin' || receiverData?.isAdmin === true;
        if (!receiverIsAdmin) {
      ...
      const unsubCall = signalingRef.current.listenToCall(newCallId, async (data) => {
        ...
        if (data.status === 'busy'    ) { setCallState(CALL_STATES.BUSY);     _reset(); }
  ...
  const answerCall = useCallback(async (incomingCallId, incomingCallData) => {
    ...
    try {
      ...
      const unsubCall = signalingRef.current.listenToCall(incomingCallId, async (data) => {
        ...
        if (data.status === 'rejected' || data.status === 'busy') { _reset(); return; }
  ...
  const rejectCall = useCallback(async (incomingCallId, status = 'rejected') => {
    try {
      await signalingRef.current?.rejectCall(incomingCallId, status);
    } catch (_) {}
  ...
  return {
    ...
    connectionState,
    iceConnectionState,
    isBusy:         callState === CALL_STATES.BUSY,
